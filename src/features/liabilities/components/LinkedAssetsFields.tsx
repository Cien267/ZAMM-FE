...
export const LinkedAssetsFields = ({
  control,
  type,
  initialPerson,
  initialCompany,
}: LinkedAssetsFieldsProps) => {
  ...
  const selectedIds = new Set(
    watchedAssets?.map((item: any) => item.assetId).filter(Boolean)
  )
                  ...
                  render={({ field }) => (
                    <FormItem className="flex w-2/3 gap-2">
                      ...
                      >
                        ...
                        <SelectContent>
                          {assetsData.map((a) => {
                            const isAlreadySelected = selectedIds.has(a.id)
                            const isCurrentValue = field.value === a.id
